        ...
        for (Map.Entry<String, Object> entry : beans.entrySet()) {
            ...
            for (Class<?> superInterface : interfaces) {
                ...
                if (superInterface == DataflowJob.class) {
                    // 获取注解
                    ElasticDataflowJob annotation =
                            instance.getClass().getAnnotation(ElasticDataflowJob.class);
                    // 获取注解里的值
                    ...
                    var jcc = JobCoreConfiguration
                            ...
                            .newBuilder(jobName, cron, sharingTotalCount)
                            .build();
                    // 2,job类型配置
                    // 参数1: 核心配置,参数2: 任务类的全类名,参数3: 是否开启定时任务(不开则只执行1次)
                    var jtc =
                            new DataflowJobConfiguration(jcc, instance.getClass().getCanonicalName(), streamingProcess);
